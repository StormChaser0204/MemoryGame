...
namespace UnityEngine.UI.Extensions
{
    ...
    public class FlowLayoutGroup : LayoutGroup
    {
            ...
            for (var i = 0; i < rectChildren.Count; i++)
            {
                ...
                if (currentBarSize + childSize > workingSize)
                {
                    ...
                    if (!layoutInput)
                    {
                        ...
                        {
                            float newOffset =
                                CalculateRowVerticalOffset(groupSize, offset, currentBarSpace);
                            LayoutRow(_itemList, currentBarSize, currentBarSpace, workingSize,
                                padding.left, newOffset, axis);
                        }
                        else if (startAxis == Axis.Vertical)
                        {
                            float newOffset =
                                CalculateColHorizontalOffset(groupSize, offset, currentBarSpace);
                            LayoutCol(_itemList, currentBarSpace, currentBarSize, workingSize,
                                newOffset, padding.top, axis);
                        }
                    }
            ...
            if (!layoutInput)
            {
                if (startAxis == Axis.Horizontal)
                {
                    ...
                    LayoutRow(_itemList, currentBarSize, currentBarSpace,
                        workingSize - (ChildForceExpandWidth ? 0 : spacingBetweenElements),
                        padding.left, newOffset, axis);
                }
                else if (startAxis == Axis.Vertical)
                {
                    ...
                    LayoutCol(_itemList, currentBarSpace, currentBarSize,
                        workingSize - (ChildForceExpandHeight ? 0 : spacingBetweenElements), newOffset,
                        padding.top, axis);
                }
            }
        ...
        private float CalculateRowVerticalOffset(float groupHeight, float yOffset,
        ...
        private float CalculateColHorizontalOffset(float groupWidth, float xOffset,
        ...
        protected void LayoutRow(IList<RectTransform> contents, float rowWidth, float rowHeight,
            float maxWidth, float xOffset, float yOffset, int axis)
        {
            var xPos = xOffset;
        ...
        protected void LayoutCol(IList<RectTransform> contents, float colWidth, float colHeight,
            float maxHeight, float xOffset, float yOffset, int axis)
        {
            var yPos = yOffset;
